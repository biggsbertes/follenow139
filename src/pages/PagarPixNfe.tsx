...
const PagarPixNfe = () => {
  ...
  useEffect(() => {
    console.log('PagarPixNfe: useEffect executado');
    
    // Verificar se há dados de usuário no localStorage
    const storedUserName = localStorage.getItem('userName');
    const storedUserCPF = localStorage.getItem('userCPF');
    
    console.log('PagarPixNfe: Dados do localStorage:', { storedUserName, storedUserCPF });
    
    if (storedUserName && storedUserCPF) {
      console.log('PagarPixNfe: Usuário encontrado no localStorage, definindo como logado');
      setUserName(storedUserName);
      setIsLoggedIn(true);
    } else {
      console.log('PagarPixNfe: Nenhum usuário encontrado no localStorage');
    }

    // Escutar mudanças no localStorage
    const handleStorageChange = (event: StorageEvent) => {
      console.log('PagarPixNfe: Storage event recebido:', event);
      const newUserName = localStorage.getItem('userName');
      const newUserCPF = localStorage.getItem('userCPF');
      
      console.log('PagarPixNfe: Novos dados do localStorage:', { newUserName, newUserCPF });
      
      if (newUserName && newUserCPF) {
        console.log('PagarPixNfe: Definindo usuário como logado via storage event');
        setUserName(newUserName);
        setIsLoggedIn(true);
      } else {
        console.log('PagarPixNfe: Definindo usuário como deslogado via storage event');
        setUserName(null);
        setIsLoggedIn(false);
      }
    };
    ...
    const handleUserLogin = (event: CustomEvent) => {
      console.log('PagarPixNfe: Evento userLogin recebido:', event.detail);
    ...
    console.log('PagarPixNfe: Event listeners adicionados');

    return () => {
      window.removeEventListener('storage', handleStorageChange);
      window.removeEventListener('userLogin', handleUserLogin as EventListener);
      console.log('PagarPixNfe: Event listeners removidos');
    };
  }, []);
  ...
  // Função para verificar o status do pagamento na API da gateway
  const checkPaymentStatus = async () => {
    ...
    try {
      ...
      if (paid) {
        ...
        navigate('/reagendamento-entrega');
      }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const code = (localStorage.getItem('lastTrackingQuery') || '').trim();
        // Carregar taxa NF-e do backend
        try {
          const cfgRes = await fetch(`${API_BASE}/api/config`);
          if (cfgRes.ok) {
            const cfg = await cfgRes.json();
            const fee = cfg?.config?.nfeFeeCents;
            if (Number.isInteger(fee) && fee >= 0) setNfeFeeCents(fee);
          }
        } catch {}
        const res = await fetch(`${API_BASE}/api/payments/pix`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ tracking: code, amountCents: amountBrl + nfeFeeCents, title: 'Taxa NF-e', category: 'nfe' })
        });
        const data = await res.json();
        if (!res.ok || data?.error) throw new Error('Falha ao gerar PIX NF-e');
  ...
  useEffect(() => {
    if (isPaymentConfirmed) return; // Para de verificar se já foi confirmado
  ...
  console.log('PagarPixNfe: Renderizando com estado:', { isLoggedIn, userName });
      ...
      {isLoading && (
        <div className="fixed inset-0 bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center z-50">
          <div className="text-center">
            ...
            <p className="text-lg font-medium text-gray-800">Gerando Pagamento Pix NF-e</p>
          </div>
        </div>
      )}
        ...
        <div className="flex justify-center">
          <div className="max-w-6xl w-full">
            ...
            <p className="text-gray-600 text-sm md:text-base mb-8 max-w-2xl">
              Pague a taxa da Nota Fiscal Eletrônica com PIX a qualquer dia e hora! O pagamento é instantâneo, prático e concluído em poucos segundos, rápido e seguro.
            </p>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="flex items-center gap-2 ml-0">
                          <img 
                            width="32" 
                            height="32" 
                            src="https://img.icons8.com/windows/32/qr-code.png" 
                            alt="qr-code"
                            className="w-5 h-5"
                          />
                        ...
                        <div className="flex items-center gap-2 ml-0">
                          <img 
                            width="24" 
                            height="24" 
                            src="https://img.icons8.com/material-outlined/24/checkmark.png" 
                            alt="checkmark"
                            className="w-5 h-5"
                          />
                ...
                <div className="bg-white rounded-lg p-4 md:p-6 shadow-sm lg:sticky lg:top-8">
                  ...
                  <div className="space-y-2 md:space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600 text-sm md:text-base">Taxa da NF-e</span>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-semibold text-sm md:text-base">R$ {(nfeFeeCents/100).toFixed(2)}</span>
                    </div>
                    
                    <div className="border-t pt-2 md:pt-3">
                      <div className="flex justify-between items-center">
                        <span className="text-base md:text-lg font-semibold text-gray-900">Total</span>
                        <span className="text-lg md:text-xl font-bold text-blue-600">R$ {((amountBrl + nfeFeeCents)/100).toFixed(2)}</span>
                      </div>
                    </div>
