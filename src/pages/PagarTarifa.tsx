...
const PagarTarifa = () => {
  ...
  useEffect(() => {
    console.log('PagarTarifa: useEffect executado');
    
    // Verificar se há dados de usuário no localStorage
    const storedUserName = localStorage.getItem('userName');
    const storedUserCPF = localStorage.getItem('userCPF');
    
    console.log('PagarTarifa: Dados do localStorage:', { storedUserName, storedUserCPF });
    
    if (storedUserName && storedUserCPF) {
      console.log('PagarTarifa: Usuário encontrado no localStorage, definindo como logado');
      setUserName(storedUserName);
      setIsLoggedIn(true);
    } else {
      console.log('PagarTarifa: Nenhum usuário encontrado no localStorage');
    }

    // Carregar tracking e tarifa
    ...
    const handleStorageChange = (event: StorageEvent) => {
      console.log('PagarTarifa: Storage event recebido:', event);
      const newUserName = localStorage.getItem('userName');
      const newUserCPF = localStorage.getItem('userCPF');
      
      console.log('PagarTarifa: Novos dados do localStorage:', { newUserName, newUserCPF });
      
      if (newUserName && newUserCPF) {
        console.log('PagarTarifa: Definindo usuário como logado via storage event');
        setUserName(newUserName);
        setIsLoggedIn(true);
      } else {
        console.log('PagarTarifa: Definindo usuário como deslogado via storage event');
        setUserName(null);
        setIsLoggedIn(false);
      }
    };
    ...
    const handleUserLogin = (event: CustomEvent) => {
      console.log('PagarTarifa: Evento userLogin recebido:', event.detail);
    ...
    console.log('PagarTarifa: Event listeners adicionados');

    return () => {
      window.removeEventListener('storage', handleStorageChange);
      window.removeEventListener('userLogin', handleUserLogin as EventListener);
      console.log('PagarTarifa: Event listeners removidos');
    };
  }, []);
  ...
  const paymentMethods = [
    ...
    },
    {
      ...
      icon: FileText,
      description: "Pagamento somente à vista",
      color: "text-blue-600"
    },
    {
      id: "pix",
  ...
  console.log('PagarTarifa: Renderizando com estado:', { isLoggedIn, userName });
            ...
            <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
              ...
              <div className="flex-1 max-w-2xl order-2 lg:order-1">
                <div className="space-y-3 md:space-y-4">
                  {/* Cartão de Crédito */}
                  <div
                    className={`bg-white rounded-lg p-4 md:p-6 cursor-pointer border-2 transition-all duration-200 hover:shadow-md ${
                      selectedPayment === "credit-card"
                        ? 'border-blue-500 shadow-md'
                        : 'border-gray-200 hover:border-gray-300'
                    }`}
                    onClick={() => { setSelectedPayment("credit-card"); navigate("/checkout-cartao"); }}
                  >
                    <div className="flex items-center gap-3 md:gap-4">
                      <div className="p-2 md:p-3 rounded-lg">
                        <img width="40" height="40" className="md:w-[50px] md:h-[50px]" src="https://img.icons8.com/ios/50/bank-card-back-side--v1.png" alt="bank-card-back-side--v1" />
                      </div>
                      <div className="flex-1">
                        <h3 className="font-semibold text-gray-900 text-base md:text-lg">Cartão de Crédito</h3>
                        <p className="text-gray-500 text-xs md:text-sm mt-1">Pague com cartão de crédito</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Boleto Bancário */}
                  <div
                    className="bg-gray-100 rounded-lg p-4 md:p-6 border-2 border-gray-200 opacity-50 cursor-not-allowed"
                  >
                    <div className="flex items-center gap-3 md:gap-4">
                      <div className="p-2 md:p-3 rounded-lg">
                        <img width="40" height="40" className="md:w-[48px] md:h-[48px]" src="https://cdn-icons-png.flaticon.com/512/25/25350.png" alt="boleto" />
                      </div>
                      <div className="flex-1">
                  ...
                  <div
                    ...
                    onClick={async () => {
                      try {
                        ...
                        setPixUrl(data?.payment?.secureUrl || '');
                        setPixQr(data?.payment?.qrcode || '');
                ...
                <div className="bg-white rounded-lg p-4 md:p-6 shadow-sm lg:sticky lg:top-8">
                  ...
                  {selectedPayment === 'pix' && (
                    <div className="mt-4 space-y-2">
                      {pixUrl && (
                        <a className="text-blue-600 underline break-all" href={pixUrl} target="_blank" rel="noreferrer">Abrir checkout</a>
                      )}
                      {pixQr && (
                        <div className="text-xs break-all p-2 bg-gray-50 rounded border">{pixQr}</div>
                      )}
                    </div>
                  )}
                </div>
